...
import arcgisscripting, math, random, os, time
...
def addFields(table):
    ...
    # Add fields to shape metrics table...
    gp.addfield(table, "ID", "short")

    gp.addfield(table, "Cohesion", "double")
    gp.addfield(table, "Proximity", "double")
    gp.addfield(table, "Spin", "double")
    gp.addfield(table, "AreaExchge", "double")
    gp.addfield(table, "PerimIndex", "double")
    gp.addfield(table, "Depth", "double")
    gp.addfield(table, "Dispersion", "double")
    gp.addfield(table, "RangeIndex", "double")
    gp.addfield(table, "Girth", "double")
    gp.addfield(table, "Detour", "double")
...
def ConvertToGridPnts(ShpGeom, gridTxtFile):
    ...
    extent = ShpGeom.Extent       # extent of shape
    # ZD extent je object no string extent.XMin, YMin XMax YMax,
    # next 2 rows are disabled
    #extent = extent.split(" ")
    # get bounding coordinates for shape
    #extent = [float(extent[0]),float(extent[1]),float(extent[2]),float(extent[3])]

    ## vertex offset required to shift points to origin...
    #x_offset, y_offset = extent[0]+1, extent[1]+1
    x_offset, y_offset = extent.XMin+1, extent.YMin+1

    # Orig:
    #extent = [extent[0]-x_offset,extent[1]-y_offset,extent[2]-x_offset,extent[3]-y_offset]

    XMin = extent.XMin-x_offset
    YMin = extent.YMin-y_offset
    XMax = extent.XMax-x_offset
    YMax = extent.YMax-y_offset
    ...
    partArray = ShpGeom.GetPart(0)  # get part array...

    pnt = partArray.next()  # get point from array

    # for each part array...
    while pnt:

        X, Y = pnt.X, pnt.Y     # get point XY
        #---------------------
        ## SHIFT COORDINATES TOWARD ORIGIN
        X -= x_offset
        Y -= y_offset
        #---------------------

        subVLst.append([X,Y])  # add XY to list
        pnt = partArray.next()  # get next point
    ...
    # rings?
    ringNo = 0
    for ringLst in pntLst:
        if ringNo > 0:
            ringLst.reverse()
        ringNo +=1

    #---------------------------
    # CONVERT SHAPE TO GRID TEXT FILE...
...
def bounds(studyArea_pntLst,Y):
    ...
    for ringLst in studyArea_pntLst:
        ...
        for pos in xrange(len(ringLst)-1):
...
def pntInShp(pntLst,testPnt):
    ...
    for ringList in pntLst:
        ...
        for i in xrange(len(ringList)-1):
