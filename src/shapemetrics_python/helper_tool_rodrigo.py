...
def ConvertToGridPnts(ShpGeom):
    ...
    #extent = [extent[0] - x_offset, extent[1] - y_offset,
    #          extent[2] - x_offset, extent[3] - y_offset]

    #print('extent - offset:', extent)
    ...
    partArray = iter(ShpGeom.exterior.coords)    # get first polygon if we have a multipolygon

    pnt = next(partArray)  # get point from array
    # for each part array...
    while pnt:
        ...
        try:
            pnt = next(partArray)  # get next point
        except StopIteration:
            break

        # why whould pnt be None?
    ...
    return pntLst, featPntLst, cellsize
    #, x_offset, y_offset
